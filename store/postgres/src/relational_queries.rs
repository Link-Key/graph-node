...
use graph::prelude::futures03::stream::Collect;
...
impl<'a> QueryFragment<Pg> for TableJoinCollection<'a> {
    fn walk_ast(&self, mut out: AstPass<Pg>) -> QueryResult<()> {
        ...
        for table_join in &self.joins {
            ...
            out.push_sql(" on (");
            out.push_sql(format!("{}.", table_join.child_prefix).as_str());
            out.push_identifier(table_join.child_column.name.as_str())?;
            out.push_sql(" = ");
            out.push_sql(format!("{}.", table_join.parent_prefix).as_str());
            out.push_identifier(table_join.parent_column.name.as_str())?;
            out.push_sql(" )");
        }
